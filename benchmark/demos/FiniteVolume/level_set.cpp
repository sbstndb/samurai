...
int level_set(std::size_t max_level, double Tf)
{
    samurai::initialize();
...
static void CASE_level_set_little(benchmark::State& state){
        for (auto _ : state){
                level_set(7, 1.0);
        }
}

static void CASE_level_set_medium(benchmark::State& state){
        for (auto _ : state){
                level_set(10, 0.05);
        }
}

static void CASE_level_set_large(benchmark::State& state){
        for (auto _ : state){
                level_set(13, 0.0005);
        }
}
...
BENCHMARK(CASE_level_set_little)
        ->Repetitions(1)
...
BENCHMARK(CASE_level_set_medium)
        ->Repetitions(1)
        ->ComputeStatistics("max", [](const std::vector<double>& v) -> double {
                return *(std::max_element(std::begin(v), std::end(v)));
        });

BENCHMARK(CASE_level_set_large)
        ->Repetitions(1)
        ->ComputeStatistics("max", [](const std::vector<double>& v) -> double {
                return *(std::max_element(std::begin(v), std::end(v)));
        });
