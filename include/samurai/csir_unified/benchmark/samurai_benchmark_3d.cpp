    ...
    for (int z = 0; z < size; ++z) {
        ...
        for (int y = 0; y < size; ++y) {
            if (dis(gen) < density) {
                ...
                if (last_x != -1) lca.add_interval_back({last_x, size}, yz);
            }
        }
    }
    return lca;
...
static void BM_SAMURAI3D_Intersection_Solid(benchmark::State& state) {
    ...
    for (auto _ : state) {
        ...
        volatile int flag = 0;
        samurai::apply(subset, [&](const auto& interval, const auto& yz) {
...
static void BM_SAMURAI3D_TurboCombo(benchmark::State& state) {
    ...
    xt::xtensor_fixed<int, xt::xshape<3>> t; t[0] = 2; t[1] = -1; t[2] = 3;
    ...
    for (auto _ : state) {
        ...
        volatile int flag = 0;
        samurai::apply(final_subset, [&](const auto& interval, const auto& yz) {
            flag ^= (interval.end - interval.start) + yz[0] + yz[1];
            return; // force eval
        });
        benchmark::DoNotOptimize(flag);
