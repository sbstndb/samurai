...
void bind_mesh_base_common_methods(py::class_<Mesh>& cls)
{
    ...
    cls.def_property_readonly("origin_point", &Mesh::origin_point, "Origin point of the mesh domain");

    cls.def_property_readonly("scaling_factor", &Mesh::scaling_factor, "Scaling factor for coordinates");
...
void bind_mr_mesh(py::module_& m, const std::string& name)
{
    ...
    cls.def(py::init(
                [](const Box& box, const samurai::mesh_config<dim>& user_config)
                {
