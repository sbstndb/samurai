...
TEST(CSIR2DTest, ExpandX_MergeNearlyTouching)
{
    ...
    CSIR_Level src; src.level = 3; src.intervals_ptr.push_back(0);
    ...
    CSIR_Level expected; expected.level = 3; expected.intervals_ptr.push_back(0);
...
TEST(CSIR2DTest, ExpandY_MergeNearlyTouching)
{
    ...
    CSIR_Level src; src.level = 2; src.intervals_ptr.push_back(0);
...
TEST(CSIR2DTest, Expand_ComposesWithUpscaleXOnly)
{
    ...
    std::size_t target = 3; int scale = 1 << (target - s.level);
...
{
    CSIR_Level s; s.level = 5; s.intervals_ptr.push_back(0);
    for (int y = -16; y < -12; ++y) { s.y_coords.push_back(y); s.intervals.push_back({8, 16}); s.intervals_ptr.push_back(s.intervals.size()); }
