...
int main()
{
    ...
    std::cout << "\nTranslate +3:" << std::endl;
    print_level_1d(moved);

    auto eroded = contract(s, 2);
    std::cout << "\nContract width=2:" << std::endl;
    print_level_1d(eroded);

    auto dilated = expand(s, 2);
    std::cout << "\nExpand width=2:" << std::endl;
    print_level_1d(dilated);

    auto up = project_to_level(s, 5);
    std::cout << "\nProject up to L5:" << std::endl;
    print_level_1d(up);

    auto down = project_to_level(up, 4);
    std::cout << "\nProject back to L4:" << std::endl;
    print_level_1d(down);
