...
namespace csir
{
    ...
    // --- Union (Optimized) ---
    inline void union_1d(std::vector<Interval>::const_iterator a_begin,
                         std::vector<Interval>::const_iterator a_end,
                         std::vector<Interval>::const_iterator b_begin,
                         std::vector<Interval>::const_iterator b_end,
                         std::vector<Interval>& result)
    {
        ...
        if (a_begin == a_end)
        {
            result.insert(result.end(), b_begin, b_end);
            return;
        }
        if (b_begin == b_end)
        {
            result.insert(result.end(), a_begin, a_end);
            return;
        }
    ...
    inline CSIR_Level union_(const CSIR_Level& a, const CSIR_Level& b)
    {
        ...
        while (it_a < a.y_coords.size() || it_b < b.y_coords.size())
        {
            ...
            else
            {
                ...
                union_1d(a_first, a_last, b_first, b_last, result.intervals);
                it_a++;
                it_b++;
            }
    ...
    inline void difference_1d(std::vector<Interval>::const_iterator a_begin,
                              std::vector<Interval>::const_iterator a_end,
                              std::vector<Interval>::const_iterator b_begin,
                              std::vector<Interval>::const_iterator b_end,
                              std::vector<Interval>& result)
    ...
    inline CSIR_Level difference(const CSIR_Level& a, const CSIR_Level& b)
    {
        ...
        while (it_a < a.y_coords.size())
        {
            ...
            if (it_b < b.y_coords.size() && b.y_coords[it_b] == y)
            {
                ...
                auto b_first = b.intervals.begin() + static_cast<std::ptrdiff_t>(b.intervals_ptr[it_b]);
                auto b_last  = b.intervals.begin() + static_cast<std::ptrdiff_t>(b.intervals_ptr[it_b + 1]);
                difference_1d(a_first, a_last, b_first, b_last, result.intervals);
            }
            else
            {
                auto first = a.intervals.begin() + static_cast<std::ptrdiff_t>(a.intervals_ptr[it_a]);
    ...
    inline void print_level_1d(const CSIR_Level_1D& csir)
    {
        ...
        std::cout << "    ";
        for (const auto& itv : csir.intervals)
        {
            std::cout << "[" << itv.start << ", " << itv.end << ") ";
        }
        std::cout << std::endl;
    }
    ...
    inline CSIR_Level_1D intersection(const CSIR_Level_1D& a, const CSIR_Level_1D& b)
    {
        CSIR_Level_1D out;
        out.level = a.level;
        if (a.level != b.level || a.empty() || b.empty())
        {
            return out;
        }
        ...
        while (ia != a.intervals.end() && ib != b.intervals.end())
        {
            ...
            if (s < e)
            {
                out.intervals.emplace_back(Interval{s, e});
            }
            if (ia->end < ib->end)
            {
                ++ia;
    ...
    inline CSIR_Level_1D project_to_level(const CSIR_Level_1D& source, std::size_t target_level)
    {
        ...
        else
        {
            ...
            if (!tmp.empty())
            {
                ...
                {
                    if (tmp[i].start <= cur.end)
                    {
                        cur.end = std::max(cur.end, tmp[i].end);
                    }
                    else
                    {
        ...
        else
        {
            ...
            for (size_t i = 0; i < source.y_coords.size(); ++i)
            {
                ...
                for (size_t k = start_idx; k < end_idx; ++k)
                {
                    ...
                    if (cs < ce)
                    {
                        bucket.emplace_back(Interval{cs, ce});
                    }
                }
            }
            if (have_group)
    ...
    inline void intersection_1d(std::vector<Interval>::const_iterator a_begin,
                                std::vector<Interval>::const_iterator a_end,
                                std::vector<Interval>::const_iterator b_begin,
                                std::vector<Interval>::const_iterator b_end,
                                std::vector<Interval>& result)
    {
        ...
        while (a_begin != a_end && b_begin != b_end)
        {
            ...
            {
                result.emplace_back(Interval{max_start, min_end});
            }
            if (a_begin->end < b_begin->end)
            {
                ++a_begin;
            }
            else
    ...
    inline CSIR_Level intersection(const CSIR_Level& a, const CSIR_Level& b)
    {
        ...
        while (it_a < a.y_coords.size() && it_b < b.y_coords.size())
        {
            ...
            else
            {
                ...
                auto b_first = b.intervals.begin() + static_cast<std::ptrdiff_t>(b.intervals_ptr[it_b]);
                auto b_last  = b.intervals.begin() + static_cast<std::ptrdiff_t>(b.intervals_ptr[it_b + 1]);
                intersection_1d(a_first, a_last, b_first, b_last, result.intervals);
    ...
    template <class TInterval>
    inline CSIR_Level
    restrict_to_domain(const CSIR_Level& set, const samurai::LevelCellArray<2, TInterval>& domain_lca, std::size_t target_level)
    {
    ...
    template <class TInterval>
    inline CSIR_Level_3D
    restrict_to_domain(const CSIR_Level_3D& set, const samurai::LevelCellArray<3, TInterval>& domain_lca, std::size_t target_level)
    {
        auto set_on_l = project_to_level(set, target_level);
        auto dom_csir = to_csir_level(domain_lca);
        auto dom_on_l = project_to_level(dom_csir, target_level);
    ...
    inline CSIR_Level_3D to_csir_level(const samurai::LevelCellArray<3, TInterval>& lca)
    {
        ...
        for (auto it = lca.cbegin(); it != lca.cend(); ++it)
        {
            ...
            if (slice.intervals_ptr.empty())
            {
                slice.intervals_ptr.push_back(0);
            }
