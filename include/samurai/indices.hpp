...
namespace samurai
{
    ...
    inline auto get_index_start_children(const Mesh& mesh, typename Mesh::mesh_interval_t& mesh_interval, const Vector& translation_vect)
    {
        ...
        xt::xtensor_fixed<value_t, xt::xshape<dim>> coord;
        coord[0] = 2 * mesh_interval.i.start + translation_vect[0];
        for (std::size_t d = 1; d < dim; ++d)
        {
            coord[d] = 2 * mesh_interval.index[d - 1] + translation_vect[d];
        }
    ...
    inline void for_each_projection_ghost(const Mesh& mesh, Func&& f)
    {
        ...
        for (std::size_t level = min_level; level < max_level; ++level)
        {
            ...
            auto cells_on_l    = csir::project_to_level(cells_p1_csir, level);
            auto inter_csir    = csir::intersection(cg_csir, cells_on_l);
            auto set_lca       = csir::from_csir_level(inter_csir, mesh.origin_point(), mesh.scaling_factor());
