...
namespace samurai::neuromesh
{
    ...
    class RewardEngine
    {
      ...
      public:
        ...
        double compute_reward(const field_t& field, double current_error, std::size_t current_cell_count) const
        {
            ...
            double reward = m_config.reward_accuracy * accuracy_reward + m_config.reward_efficiency * efficiency_reward
      ...
      private:
        ...
        double compute_efficiency_reward(std::size_t current_cell_count) const
        {
            ...
            double cell_ratio = static_cast<double>(current_cell_count) / static_cast<double>(m_previous_cell_count);
    ...
    class RLAgent
    {
      ...
      public:
        ...
        explicit RLAgent(const config_t& config = config_t{}, std::size_t feature_dim = 5)
            : m_config(config)
            , m_rng(std::random_device{}())
        ...
        void
        store_experience(const feature_array_t& state, CellAction action, double reward, const feature_array_t& next_state, bool done = false)
        {
            Experience exp;
            exp.state      = state;
      ...
      private:
        ...
        {
            std::uniform_int_distribution<int> dist(0, static_cast<int>(CellAction::Count) - 1);
        ...
        CellAction action_with_max_q(const feature_array_t& state)
        {
            ...
            return static_cast<CellAction>(static_cast<int>(xt::argmax(q_values)()));
        }
        ...
        std::vector<std::size_t> sample_batch_indices()
        {
            ...
            std::uniform_int_distribution<std::size_t> dist(0, m_replay_buffer.size() - 1);
    ...
    class AdaptationController
    {
      ...
      public:
        ...
        void adapt(field_t& field, double target_error)
        {
            ...
            double reward                  = m_reward_engine.compute_reward(field, m_current_error, current_cell_count);

            // Store experience for training
            m_reward_engine.update_state(m_current_error, current_cell_count);
            m_rl_agent.store_experience(features,         // state (simplified - should store previous state)
