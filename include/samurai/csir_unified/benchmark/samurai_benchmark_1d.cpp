...
namespace
{
    ...
    static samurai::LevelCellArray<1>
    make_random_lca_1d(std::size_t level, int x_min, int x_max, int n_intervals, int min_len = 2, int max_len = 15, unsigned seed = 42)
    {
        ...
        for (int k = 0; k < n_intervals; ++k)
        {
            ...
            if (s < e)
            {
                tmp.emplace_back(s, e);
            }
        }
        std::sort(tmp.begin(),
                  tmp.end(),
                  [](const interval_t& a, const interval_t& b)
                  {
...
static void BM_SAMURAI1D_Translate(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        xt::xtensor_fixed<int, xt::xshape<1>> t;
        t[0]              = 10;
