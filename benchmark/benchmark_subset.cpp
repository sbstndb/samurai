...
void SUBSET_difference_same_interval_different_level(benchmark::State& state)
{
    ...
    for (int64_t i = 0; i < state.range(0); i++)
    {
        ...
        cl2[delta_level][{}].add_interval({static_cast<int>(pow(2, delta_level + 1)) * index,
                                           static_cast<int>(pow(2, delta_level + 1)) * index + static_cast<int>(pow(2, delta_level))});
    }
...
template <unsigned int dim, unsigned int delta_level>
void SUBSET_difference_same_interval_different_level_SOA(benchmark::State& state)
{
    samurai::CellList<dim> cl1, cl2;
    for (int64_t i = 0; i < state.range(0); i++)
    {
    ...
    samurai::CellArray<dim> ca1(cl1);
    samurai::CellArray<dim> ca2(cl2);
    ca1[0].convertAOStoSOA();
    ca2[0].convertAOStoSOA();
    for (auto _ : state)
    {
        auto subset            = samurai::difference(ca1[0], ca2[0]);
        volatile int evaluated = 0;
...
void SUBSET_intersection_same_interval_different_level(benchmark::State& state)
{
    ...
    for (auto _ : state)
    {
        ...
        if constexpr (delta_level != 0)
        {
            //                        auto subset = samurai::intersection(ca1[0], ca2[delta_level].on(0));
            //                        subset([](const auto&, const auto&) {}); // evaluation avec lambda vide
            //                        benchmark::DoNotOptimize(subset);
        }
        else if constexpr (delta_level == 0)
...
BENCHMARK_TEMPLATE(SUBSET_difference_same_interval_different_level_SOA, 1, 0)->RangeMultiplier(2)->Range(1 << 1, 1 << 10);
// BENCHMARK_TEMPLATE(SUBSET_difference_same_interval_different_level_SOA,2, 0)->RangeMultiplier(2)->Range(1 << 1, 1 << 10);
// BENCHMARK_TEMPLATE(SUBSET_difference_same_interval_different_level_SOA,3, 0)->RangeMultiplier(2)->Range(1 << 1, 1 << 10);
